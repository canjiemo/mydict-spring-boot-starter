...
当前版本：`1.0.4-jdk21`，支持 JDK 21+ 和 Spring Boot 3.x。
...
这个项目最早来自一个很常见的问题：前端页面需要展示字典文案，但传统做法往往都不够理想。

- 后端手工新增 `statusDesc`、`typeDesc` 等字段，并在代码里逐个赋值，重复且容易遗漏
- 前端拿到字典值后再调用字典接口自行解析，增加请求次数，也让页面逻辑分散
- 前后端各自维护一套映射关系，最终容易出现数据不一致

这些问题本质上都不是业务核心，却长期消耗开发和联调成本。

## ✅ 解决什么问题

MyDict 试图用一套更稳定的方式处理字典展示问题：

- 让后端直接返回可展示的数据，减少前端额外查字典的负担
- 让业务字段只关注“原始值”，描述字段交给编译期自动生成
- 让字典解析逻辑集中到统一入口，便于缓存、兜底文案和命名规则管理
- 兼容 PO、DTO、VO 甚至 MyBatis-Plus 实体上的常见使用场景

如果你的项目里经常出现“接口里既要返回字典值，又要返回对应说明”的需求，这个 starter 解决的就是这类问题。
